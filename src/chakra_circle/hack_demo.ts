...
function draw() {
    var screen_size = v(p.width, p.height);
    var center = screen_size.copy().div(2);
    ...
    const feature_domain = [v(1, 0), v(0, cap.width / cap.height)];
    const render_range = [
        center.copy().add(v(-1, -1).mult((canvas_scale * 1) / 3)),
        center.copy().add(v(1, 1).mult((canvas_scale * 1) / 3)),
    ];

    const draw_frame = VecScale.linear()
        .domain(feature_domain)
        .range(render_range);

    p.background(0);
    draw_view_frame(p, draw_frame);
    draw_cap_count(p, cap);
    ...
    if (chaks.close.activated) {
        feature_ring(
            p,
            chaks.close.color,
            chaks.meta.SHOULDER_CENTER,
            chaks.meta.H_TO_S.mag() * 2,
            50,
        );
    }

    draw_chakra_activation(p, chaks.coords);
}
