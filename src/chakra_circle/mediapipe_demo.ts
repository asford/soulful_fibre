...
function sketch(p: p5) {
  ...
  function draw() {
    ...
    function norm_vec(lm: NormalizedLandmark | undefined): p5.Vector {
      ...
      return p.createVector(1 - lm.x, lm.y * (current.height / current.width));
    }

    const half = p.createVector(0.5, 0.5);
    ...
    dot(
      colors.grey,
      norm_to_frame(swheel.center),
      swheel.radius * canvas_scale * 2,
      10,
    );
    p.text(
      swheel.h_angle,
    ...
    if (current.result?.poseLandmarks) {
      POSE_CONNECTIONS.forEach((edge) => {
        ...
        p.stroke(colors.straw).line(f.x, f.y, t.x, t.y);
      });
    }
